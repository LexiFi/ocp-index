...
let qualify_ty_in_sig_item (parents:parents) =
  ...
  | Osig_value oval -> Osig_value {oval with oval_type = qual oval.oval_type}
        ...
        (fun { Types.ld_id; ld_type; ld_attributes } ->
          ...
          let ty =
            Outcometree.Osig_type (Outcometree.({
                ...
                otype_unboxed = false }), Outcometree.Orec_not)
          in
          let doc = doc_of_attributes ld_attributes in
          string_to_key (Ident.name ld_id),
          Trie.create ~value:{
            path = info.path;
            orig_path = info.path;
            kind = Field info;
            name = Ident.name ld_id;
            ty = Some ty;
  ...
  | Types.Type_variant variants ->
      List.map
        (fun { Types.cd_id; cd_args; cd_attributes } ->
          ...
          string_to_key (Ident.name cd_id),
          Trie.create ~value:{
            path = info.path;
            orig_path = info.path;
            kind = Variant info;
            name = Ident.name cd_id;
            ty = Some ty;
            loc_sig = info.loc_sig;
            loc_impl = info.loc_impl;
            doc = lazy doc;
...
let rec lookup_trie_of_module_expr parents t path = function
  ...
  | Typedtree.Tmod_apply ({ mod_desc = Typedtree.Tmod_functor(id,_,_,f) },
      ...
      debug "Grafting %s at %s\n" (Ident.name id) (modpath_to_string (path_of_ocaml arg));
      let functor_arg = lazy (lookup_parents parents (path_of_ocaml arg) path) in
      Trie.graft_lazy t (modpath_to_key [Ident.name id]) functor_arg
  | _ -> t
...
let load_dirs t dirs =
  let dirfiles =
    List.fold_left (fun acc dir ->
        ...
        let files =
          List.rev_map (fun f -> dir, f) (Array.to_list (try Sys.readdir dir with Sys_error _ -> [||]))
        in
